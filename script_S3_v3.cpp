...
bool ReadOVF1(double** g, bool** b, int** Dimension, char inputfilename[100])
{
   bool p = 1;
    int Nx = Dimension[0][0];
    int Ny = Dimension[0][1];
            ...
            for (int k=0; k<Nz; k++){
                for (int j=0; j<Ny; j++){
                    for (int i=0; i<Nx; i++){
                        ...
                        sscanf(line, "%lf %lf %lf", &temp8_x,&temp8_y,&temp8_z);
                        double norm = 1./sqrt(temp8_x*temp8_x + temp8_y*temp8_y + temp8_z*temp8_z);
                        sx = temp8_x*norm; 
                        sy = temp8_y*norm;
                        sz = temp8_z*norm;
                        sv = 0.0;
                ...
                for (int k=0; k<Nz; k++){
                    for (int j=0; j<Ny; j++){
                        ...
                        if (binType==4){
                            ...
                            double norm = 1./sqrt((double)temp4_x*(double)temp4_x + (double)temp4_y*(double)temp4_y + (double)temp4_z*(double)temp4_z);
...
bool ReadOVF2(double** g, bool** b, int** Dimension, char inputfilename[100])
{
   bool p = 1;
    int Nx = Dimension[0][0];
    int Ny = Dimension[0][1];
...
void BP_ini(double** g, bool** b,int** Dimension, double** Parameters){
    ...
    for(int i = 0; i<Nx; i++){
        for(int j = 0; j<Ny; j++){
            ...
            double Th = 0.8*M_PI*((nz+1)/2)*((nz+1)/2)*((nz+1)/2);
...
auto aCopy4DNesterov(double** dst, double** src, double beta, int** Dimension, int nt){
    int Nx = Dimension[0][0];
    int Ny = Dimension[0][1];
    int Nz = Dimension[0][2];
    int NumOfShelsX = Dimension[1][0];
    ...
    for(int k = NumOfShelsZ; k < Nz/nc + NumOfShelsZ; k++){
        for(int j = NumOfShelsY; j < Ny/nb + NumOfShelsY; j++){
            for(int i = NumOfShelsX; i < Nx/na + NumOfShelsX; i++){ 
                    int pos = i+j*N1+k*N1*N2;
                    for(int q=0; q<3; q++){
                        double temp = dst[nt][pos+q*Ns];
                        dst[nt][pos+q*Ns] = dst[nt][pos+q*Ns] + beta*(dst[nt][pos+q*Ns] - src[nt][pos+q*Ns]);
                        src[nt][pos+q*Ns] = temp;
                    }
                }
            }
        }
...
void Copy4DNesterov(double** dst,double** src, double beta, int** Dimension){
    int NumOfThreads = Dimension[4][1];
    vector<future<void>> futureVec(NumOfThreads);
    for(int n = 0; n < NumOfThreads; ++n)
        futureVec[n] = async(aCopy4DNesterov, dst, src, beta, Dimension, n);
    for(int n = 0; n < NumOfThreads; ++n)
        futureVec[n].get();
}

auto aCopy4D(double** dst,double** src,int** Dimension, int nt){
    int Nx = Dimension[0][0];
    int Ny = Dimension[0][1];
    int Nz = Dimension[0][2];
    int NumOfShelsX = Dimension[1][0];
    ...
    for(int k = NumOfShelsZ; k < Nz/nc + NumOfShelsZ; k++){
        for(int j = NumOfShelsY; j < Ny/nb + NumOfShelsY; j++){
            for(int i = NumOfShelsX; i < Nx/na + NumOfShelsX; i++){ 
                    int pos = i+j*N1+k*N1*N2;
                    dst[nt][pos]      = src[nt][pos];
                    dst[nt][pos+Ns]   = src[nt][pos+Ns];
                    dst[nt][pos+2*Ns] = src[nt][pos+2*Ns];
                }
            }
        }
}

void Copy4D(double** dst,double** src,int** Dimension){
    int NumOfThreads = Dimension[4][1];
    vector<future<void>> futureVec(NumOfThreads);
    for(int n = 0; n < NumOfThreads; ++n)
        futureVec[n] = async(aCopy4D,dst,src,Dimension,n);
    for(int n = 0; n < NumOfThreads; ++n)
        futureVec[n].get();
}

auto MoveAlongGradGB4D_NCG(double** g, bool** b,double** de, bool* remap,double step, int** Dimension, int nt){
    double g1, g2, g3;
    remap[nt] = true;
   int Nx = Dimension[0][0];
   int Ny = Dimension[0][1];
...
void MakeOneStepGB4D_NCG(double** g, bool** b, double** de, bool* remap, double step, int** Dimension){

   int NumOfThreads = Dimension[4][1];
   vector<future<void>> futureVec(NumOfThreads);
   for(int n = 0; n < NumOfThreads; ++n)
         futureVec[n] = async(MoveAlongGradGB4D_NCG,g,b,de,remap,step,Dimension,n);
      for(int n = 0; n < NumOfThreads; ++n)
         futureVec[n].get();
}

auto remapAll4D(double** g,bool** b,int** Dimension, int nt){
    int Nx = Dimension[0][0];
    int Ny = Dimension[0][1];
    int Nz = Dimension[0][2];
    int NumOfShelsX = Dimension[1][0];
    ...
    int Ns = N1*N2*N3;
    double g1, g2,g3, ss, nx, ny, nz,nw, gg, gi;
    for(int k = NumOfShelsZ; k < Nz/nc + NumOfShelsZ; k++){
        for(int j = NumOfShelsY; j < Ny/nb + NumOfShelsY; j++){
            for(int i = NumOfShelsX; i < Nx/na + NumOfShelsX; i++){
                    ...
                    gg = g1*g1+g2*g2+g3*g3;
                    gi = 1./(1.+gg);
                    nx = 2.*g1*gi; ny = 2.*g2*gi; nz = 2.*g3*gi; nw = ss*(1.-gg)*gi;

                    ss = (nw>0)? 1.0 : -1.0;
                    g[nt][pos]      = nx/(1.0 + ss*nw);
                    g[nt][pos+Ns]   = ny/(1.0 + ss*nw);
                    g[nt][pos+2*Ns] = nz/(1.0 + ss*nw);
                    b[nt][pos]    = (nw>0)? true : false;
...
void RemapAll4D(double** g, bool** b, int** Dimension){
    int NumOfThreads = Dimension[4][1];
    vector<future<void>> futureVec(NumOfThreads);
    for(int n = 0; n < NumOfThreads; ++n)
        futureVec[n] = async(remapAll4D,g,b,Dimension,n);
    for(int n = 0; n < NumOfThreads; ++n)
        futureVec[n].get();
}


void Nesterov(double** g, bool** b, double** g0, double** de, bool* remap, int** Dimension, double** Parameters, double* JD){
    int NumOfThreads = Dimension[4][1];
    int NumOfIter = 1+Dimension[5][1];
    int Iter = Dimension[5][2];
    int Nx = Dimension[0][0];
    int NumOfShelsX = Dimension[1][0];
    int na = Dimension[2][0];
    ...
    double al = Parameters[5][1], e0=1.0e10, energy;
    double grad, tol = Parameters[5][2];
    double dV = Parameters[4][0]*Parameters[4][1]*Parameters[4][2]/(Dimension[0][0]*Dimension[0][1]*Dimension[0][2]);
    ...
    ///SD with optimal step
    GetMagnetization_4D(g,b,Dimension);
    Mx = g[0][0]*dV; My = g[0][Ns]*dV; Mz = g[0][2*Ns]*dV; Mv = g[0][Nx/na + NumOfShelsX]*dV;
    UpdateBoundaryGB(g,b,Dimension,3);
    energy = GetEnergyGB_4D(g,b,Dimension,Parameters,JD);
    printf("Initial State Energy: %0.10f, M: [%f, %f, %f, %0.3e]\n", energy, Mx, My, Mz, Mv);
    printf("\n");

    //One SD step
    

    for(int k = 1; k<NumOfIter; k++){
        
        double beta = (1.0*k - 1.0)/(2.0 + 1.0*k);
        if(k == 1){
            Copy4D(g0, g, Dimension);
        }else{
            Copy4DNesterov(g, g0, beta, Dimension);
        }
        UpdateBoundaryGB(g, b, Dimension,3);
        grad = GetEffectiveFieldGB_4D(g, b, de, Dimension, Parameters, JD);
        ...
        MakeOneStepGB4D_NCG(g,b,de,remap,al,Dimension);
        UpdateBoundaryGB(g,b,Dimension,3);
        //check remap
        NeedRemap = false;
        for(int n=0; n<NumOfThreads; n++){
        ...
        if((k-1)%Iter==0){
            ...
            printf("%d, %0.10f, %0.3e, %f, %f, %f, %0.3e, %0.3e, %0.3e\n", (k-1)/Iter, energy, grad, Mx, My, Mz, Mv, al, beta);
            if(Dimension[6][1]){
                pFile = fopen (ConvergenceInfoFile,"a+");
                snprintf(BuferString,400,"%d, %0.20f, %0.20f, %0.20f, %0.20f, %0.20f, %0.20f\n",(k-1)/Iter,energy, grad,Mx,My,Mz,Mv);
                fputs (BuferString,pFile);
                fclose (pFile);
            }
            if(Dimension[7][0]) WriteData2OVF(g,b,Dimension);
    ...
    GetMagnetization_4D(g,b,Dimension);
    Mx = g[0][0]*dV; My = g[0][Ns]*dV; Mz = g[0][2*Ns]*dV; Mv = g[0][Nx/na + NumOfShelsX]*dV;
    printf("Final State Energy: %0.10f, M: [%f, %f, %f, %0.3e]\n", energy, Mx, My, Mz, Mv);
    WriteData2OVF(g,b,Dimension);
}
...
if(Dimension[6][0]==1){
    ...
    if(Dimension[5][0]){
        ...
        bool success = ReadOVF2(G,B,Dimension, inputfilename);
        free(inputfilename);

        if(!success){
            char *inputfilename1 = NULL;
            int p = asprintf(&inputfilename1, "%s.ovf", InputOvf);  
            printf("Read initial state from %s\n", inputfilename1);
            success = ReadOVF1(G,B,Dimension, inputfilename1);
            free(inputfilename1);
        }
        if(!success){
            printf("Cant open %s.ovf and %s.ovf2\n", InputOvf, InputOvf);
            exit(3);
        }
    }else{
    ...
    for(int i = 0; i < NumOfThreads; ++i)
        delete [] G[i];
    delete [] G;
}else if(Dimension[6][0]==2){
    printf("A bit faster method.\n");
    double **DE  = new double*[NumOfThreads];
    double **G0   = new double*[NumOfThreads];
    double **G   = new double*[NumOfThreads];
    bool   **B   = new bool*[NumOfThreads];
    bool *Remap = new bool[NumOfThreads];

        
    for(int i = 0; i < NumOfThreads; ++i){
        DE[i] = new double[3*Ns];
        G0[i] = new double[3*Ns];
        G[i] = new double[3*Ns];
        B[i] = new bool[Ns];
        Remap[i] = false;
    }
    ...
    start = chrono::system_clock::now();
    Nesterov(G, B, G0, DE, Remap, Dimension, Parameters, JD);
    stop = chrono::system_clock::now();
    time += chrono::duration<double>(stop-start).count();
    printf("Total time: %f\n",time);
